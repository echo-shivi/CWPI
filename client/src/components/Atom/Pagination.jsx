...
const Pagination = ({ currentPage, totalPages, firstIndex, onPageChange, lastIndex, pages, prevPage, nextPage, totalEntries }) => {
  return (
    <nav className=" flex justify-between pt-10" aria-label="Pagination">
      ...
      <ul className='flex'>
        <li onClick={prevPage} className="relative inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-pointer rounded-l-md">
        ...
        </li>
        {
          pages.map((page, index) => (
            <li key={index} onClick={() => onPageChange(page)} className={`${currentPage === totalPages ? 'active' : ''} relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 cursor-pointer border  ${currentPage === page ? 'bg-blue-300 border-blue-300' : 'bg-white border-gray-300'}`}>
              {page}
        ...
        <li onClick={nextPage} className="relative cursor-pointer inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-r-md">
