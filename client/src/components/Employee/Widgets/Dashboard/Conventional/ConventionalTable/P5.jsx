...
const CompletedScheme = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}employee/conventionalP5/details`);
                setTablesData(response.data.conventionalP5);
                console.log('API response:', response.data.conventionalP5);
            } catch (error) {
                ...
                <table className="table-auto w-full">
                    <thead>
                        <tr>
                            ...
                            <th className="border h-16 text-base text-center bg-blue-400 text-white">Date Of Completion As Per Agreement</th>
                            <th className="border h-16 text-base text-center bg-blue-400 text-white">Actual Date Of Completion</th>
                            <th className="border h-16 text-base text-center bg-blue-400 text-white">Spillage</th>
                        ...
                        {currentEntries.length > 0 ? (
                            currentEntries.map((entry, index) => (
                                ...
                                >
                                    ...
                                    <td className="border font-medium">{entry?.Date_Of_Completion_As_Per_Agreement}</td>
                                    <td className="border font-medium">{entry?.Actual_Date_Of_Completion}</td>
                                    <td className="border font-medium">{entry?.Spillage}</td>
