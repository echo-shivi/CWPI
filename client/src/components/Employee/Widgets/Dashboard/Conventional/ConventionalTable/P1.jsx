...
const CompletedScheme = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}employee/conventionalP1/details`);
                setTablesData(response.data.conventionalP1);
                console.log('API response:', response.data.conventionalP1);
            } catch (error) {
                ...
                <table className="table-auto w-full">
                    <thead>
                        <tr>
                            ...
                            <th className="border h-16 text-base text-center bg-blue-400 text-white">Date Of Administrative Approval</th>
                            <th className="border h-16 text-base text-center bg-blue-400 text-white">Date Of Technical Sanction</th>
                        ...
                        {currentEntries.length > 0 ? (
                            currentEntries.map((entry, index) => (
                                ...
                                >
                                    ...
                                    <td className="border font-medium">{entry?.Date_Of_Administrative_Approval}</td>
                                    <td className="border font-medium">{entry?.Date_Of_Technical_Sanction}</td>
