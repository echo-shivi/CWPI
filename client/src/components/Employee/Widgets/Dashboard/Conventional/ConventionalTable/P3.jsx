...
const CompletedScheme = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}employee/conventionalP3/details`);
                setTablesData(response.data.conventionalP3);
                console.log('API response:', response.data.conventionalP3);
            } catch (error) {
                ...
                <table className="table-auto w-full">
                    <thead>
                        <tr>
                            ...
                            <th className="border h-16 text-base text-center bg-blue-400 text-white">Date Of Tender Finalisation </th>
                        ...
                        {currentEntries.length > 0 ? (
                            currentEntries.map((entry, index) => (
                                ...
                                >
                                    ...
                                    <td className="border font-medium">{entry?.Date_Of_Tender_Finalisation}</td>
