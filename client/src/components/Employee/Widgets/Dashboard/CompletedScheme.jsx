...
const CompletedScheme = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}employee/completedSchemes/details`);
                setTablesData(response.data.schemes_names);
                console.log('API response:', response.data.schemes_names);
            } catch (error) {
    ...
    const filteredEntries = useMemo(() => {
        return searchTerm
            ? tablesData.filter(
                (entry) =>
                    entry.schemes_name.toLowerCase().includes(searchTerm.toLowerCase()))
            : tablesData;
                ...
                <table className="table-auto w-full">
                    <thead>
                        <tr>
                            ...
                            <th className="border h-16 text-base text-center bg-blue-400 text-white">Access Level</th>
                            <th className="border h-16 text-base text-center bg-blue-400 text-white">CWPI Format</th>
                        ...
                        {currentEntries.length > 0 ? (
                            currentEntries.map((entry, index) => (
                                ...
                                >
                                    ...
                                    <td className="border font-medium">{entry?.schemes_name}</td>
                                    <td className="border font-medium">{entry?.access_level}</td>
                                    <td className="border font-medium">{entry?.cwpi_format}</td>
