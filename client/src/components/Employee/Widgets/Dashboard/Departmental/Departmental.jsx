...
const SchemeReport = () => {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_BASE_URL}employee/departmental/details`)
      .then((response) => {
        console.log('API response:', response.data.departmentalCards);
        setCardsData(response.data.departmentalCards);
      })
      .catch((error) => {
