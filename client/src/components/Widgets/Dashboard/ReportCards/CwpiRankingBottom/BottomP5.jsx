...
const Bottom5 = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}dashboard/reportCard/reportDetails/cwpiRankingBottom/bottom5/details`);
                setTablesData(response.data.bottom5);
                console.log('API response:', response.data.bottom5);
            } catch (error) {
    ...
    return (
        <div className="container bg-white rounded-lg mx-auto p-4 mb-6" ref={pdfRef}>
        
            <div className="justify-between flex pb-6 ">
            <h1 className=" border-gray-300 text-center font-normal bg-gray-100 p-4 rounded-lg  text-gray-700 text-[22px] "> Bottom P5</h1>
